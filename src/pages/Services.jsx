            ...
            <div key={s.slug} className={`svc-row ${i % 2 === 0 ? 'bg-background' : 'bg-surface'}`}>
              <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16 py-16 sm:py-20 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
                <div className={`rounded-3xl overflow-hidden border border-divider ${imageFirst ? 'lg:order-1' : 'lg:order-2'}`}>
                  <Img src={s.img} alt={s.alt} sizes="(min-width: 1024px) 33vw, 100vw" loading="lazy" decoding="async" className="h-full w-full object-cover aspect-[4/3]" />
                </div>
                <div className={imageFirst ? 'lg:order-2' : 'lg:order-1'}>
                  ...
                  <h2 className="font-display text-2xl sm:text-3xl font-bold tracking-tighter mb-3 text-ink">{s.title}</h2>
                  <p className="text-muted leading-relaxed mb-6">{s.text}</p>
                  <Link to={`/service/${s.slug}`} className="inline-flex items-center gap-1.5 text-sm font-medium text-primary lift-on-hover">
