      ...
      <section ref={bodyRef} className="py-24 sm:py-32">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16 grid lg:grid-cols-12 gap-12">
          <div className="svc-detail-text lg:col-span-7">
            ...
            <Link to="/contact" className="magnetic-btn inline-flex items-center gap-2 bg-primary text-deep px-6 py-3 rounded-full font-semibold shadow-lg shadow-primary/30 mt-10">
            ...
            {service.gallery.length > 1 ? (
              <div className="grid grid-cols-2 gap-4">
                <div className={`rounded-3xl overflow-hidden border border-divider ${service.gallery.length >= 3 ? 'col-span-2' : ''} aspect-[4/3]`}>
                  <Img src={service.gallery[0].src} alt={service.gallery[0].alt} sizes="(min-width: 1024px) 50vw, 100vw" loading="lazy" decoding="async" className="h-full w-full object-cover" />
                </div>
      ...
      <section ref={othersRef} className="bg-deep text-white py-24 sm:py-32">
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-white/5">
            {others.map((s) => (
              <Link key={s.slug} to={`/service/${s.slug}`} className="svc-other-tile bg-deep p-8 sm:p-10 transition-colors hover:bg-white/[0.03] group">
            ...
            <Link to="/services" className="svc-other-tile bg-deep p-8 sm:p-10 transition-colors hover:bg-white/[0.03] group flex flex-col justify-center items-start">
              ...
              <p className="text-white/60 text-sm leading-relaxed">See the full list of what Fibonacci can help with.</p>
            </Link>
          </div>
        </div>
      </section>
